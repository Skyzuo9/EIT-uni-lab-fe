...
async function waitForPreview(url, preview) {
  ...
  throw new Error(`F07 前端预览未就绪: ${lastError}`)
}

/**
 * 终止一个由包装器拥有的进程组并等待真实退出。
 *
 * @param child 待关闭的构建、预览或 Playwright 子进程。
...
async function main() {
  ...
  try {
    ...
    if (buildResult !== 0) throw new Error(`F07 前端构建失败: ${buildResult}`)
