...
export default function DevicePanel(): React.JSX.Element {
  ...
  useEffect(() => {
    setArgumentDraft(
      selectedAction ? createArgumentDraft(selectedAction.inputSchema) : {}
    )
    setFormError(null)
        ...
        {selectedDevice ? (
          <DeviceWorkspace
            ...
            onArgumentChange={(name, value) =>
              setArgumentDraft((current) => ({ ...current, [name]: value }))
            }
    ...
    <div className="edge-device__execution" aria-live="polite">
      ...
      {execution.node?.result
        && Object.keys(execution.node.result).length > 0 ? (
          <pre>{JSON.stringify(execution.node.result, null, 2)}</pre>
